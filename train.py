...
from utils.learning.callbacks import PrintValidations
...
def train_model(dataset, batch_size=8, epochs=100, patience=10, input_dims=(512,512)):
    # Varibales and data generator
    input_dim_x, input_dim_y = input_dims
    n_filters = 32
    dataset = dataset
    ...
    es = EarlyStopping(monitor='val_dice_coef', patience=patience, mode='max', restore_best_weights=True)
    print_vals = PrintValidations(data_gen)
    #training_history = model.fit(x_train, y_train, batch_size=batch_size, epochs=epochs
    #                             , validation_split=0.2, verbose=1, callbacks=[])

    model.summary()
    model.compile(optimizer=Adam(learning_rate=learning_rate), loss=loss, metrics=[IoU, dice_coef, precision, recall], run_eagerly=False, steps_per_execution=4)
...
if __name__ == "__main__":
    args = sys.argv[1:]
    if len(args)<1:
        train_model('fuseg_augmented')
    else:
        train_model(args[0], int(args[1]), int(args[2]), int(args[3]))